...
In terms of the Internet traffic mix of home networks, a number of studies on
...
\todo{user engagement with network and required functionality}

\todo{expressing user collaboration on the social aspect. how user share their
  network?}
\todo{Mismatch of user perception to real properties of the network. e.g. what
  are the limits of the local network?}
...
\begin{figure} \centering \includegraphics[width=0.9\columnwidth]{architecture}
...
\begin{table} \footnotesize
  \begin{tabular}{p{0.35\columnwidth}p{0.55\columnwidth}} \textbf{Method} &
    \textbf{Function} \\ \url{permit/<eaddr>} & Permit access by specified
    client\\ \url{deny/<eaddr>} & Deny access by specified client\\
    \url{status/[eaddr]} & Retrieve currently permitted clients, or status of
    specified client \\ \url{dhcp-status/} & Retrieve current MAC--IP mappings\\
    ...
    \url{permit-dns/<e>/<d>} & Permit access to domain \texttt{d} by client
    \texttt{e}\\ \url{deny-dns/<e>/<d>} & Deny access to domain \texttt{d} by
    client \texttt{e}\\ \end{tabular} \caption{\label{t:api}Web service API;
    prefix all methods \texttt{https://.../ws.v1/}.  $<$\,$X$\,$>$ and $[X]$
    denote required and optional parameters.} \vspace{-2.5em} \end{table}
...
% , e.g.,~Tom's Laptop in Figure~\ref{f:guest-board} is currently the dominant
% bandwidth user. 
\item Wireless Ethernet retransmissions show as red highlights on the device's
  label. % , indicating devices currently
...
\begin{figure} \centering \includegraphics[width=0.9\columnwidth]{eapol}
...
\begin{figure} \centering \includegraphics[width=0.9\columnwidth]{rekeying}
  \caption{\label{f:rekeying}Affect on TCP throughput from rekeying every 30s
    ...
    Windows 7 using a proprietary Intel driver and card.} \vspace{-1.5em}
\end{figure}
...
% is forwarded to the controller (NOX) for processing, as well as a default
% packet handler for all traffic not otherwise matched.
...
\begin{figure} \centering \includegraphics[width=\columnwidth]{stack-throughput}
...
% "A workgroup that spans an IPv4 router creates two separate workgroups. With
% workgroups, there is no mechanism to propagate the list of servers collected
% by the Master Browse Server on one subnet to the Master Browse Server on
% another subnet. Master Browse Servers for workgroups do not register a special
% NetBIOS name with WINS that may be used by workgroup Master Browse Servers or
% by browse clients. There are no special Lmhosts entries that are used to
% forward unicast workgroup browse list information between Master Browse
% Servers."
...
\section{User-Driven Resource Management} \label{s:qos}
