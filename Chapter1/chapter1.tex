...
testing and performance characterisation.  \mycite{Weissmann:va} present the
...
current controller frameworks, like~\mycite{Gude08,floodlight}, are designed for
...
\mycite{oflops-manual}~provides further details regarding the \oflops programming
model.
...
module~\mycite{pktgen}~(Figure~\ref{fig:oflops_design_kernel}), and
...
specification~\mycite{openflow-spec}, the \texttt{barrier} command is a way to be
...
In~\mycite{Gupta01}, the authors characterise the update complexity of a TCAM to
be linear. 
...
the network~\mycite{plug_n_serv,Yap09}. Using \oflops advanced measurement
...
\oflops and Cbench~\mycite{cbench} provide a sufficient toolbox to profile the
...
portions of network functionality with simpler models.  \mycite{Vahdat2002}
...
and the \ns{3} backend scalability.  \mycite{madhavapeddy2013} presents an
...
NOX~\mycite{nox} is one of the first and most mature open-source \of controllers;
...
% Based on the \oflops results we evaluate a median flow insertion delay of 6msec,
% a \texttt{Pkt\_in} processing delay of 2msec and a maximum rate of 50
% \textit{Pkt\_in} messages per second. 
...
\caption[\ns{3} scalability evaluation topology]{Network topology to test the scalability of \ns{3}-based simulation. We
...
\end{tabular}
\end{center}
\caption[\sdnsim \ns{3} backend evaluation result]{Wall-clock delay to run a 30 seconds simulation time of the topology
...
We evaluate the \sdnsim simulation scalability using a simple network topology,
...
replicates the control logic presented by~\mycite{Al-Fares08}.  Every second the
